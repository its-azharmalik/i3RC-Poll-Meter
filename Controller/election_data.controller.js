...
const postElectionData = async (req,res) =>{
    ...
    try {
        ...
        const new_voter_data = await Voter.findByIdAndUpdate(
            { _id: voterid },
            {$push: { Yearly_Election_Data : { Year : req.body.Year  ,Election_Data_ID : new_election_data._id } }}
        );
        res.json({
            note : "success",
...
const updateElectionData = async (req,res) =>{
    ...
    try {
        ...
        const new_election_data = await Election_Data.findByIdAndUpdate(electiondataid , data );
        res.json({
            note : "success",
...
const deleteElectionData =  async (req,res) =>{
    ...
    try {
        const new_election_data = await Election_Data.findById(electiondataid);
        Voter.findOneAndUpdate({Yearly_Election_Data : { Election_Data_ID : electiondataid }},
            {
                $pull : {
